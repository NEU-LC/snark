...
#ifndef SNARK_BATTERY_OCEAN_TIMEOUTSERIAL_H
#define SNARK_BATTERY_OCEAN_TIMEOUTSERIAL_H
 ...
 */
class serial_io: private boost::noncopyable
{
public:
    serial_io();
     ...
     * serial device
     */
    serial_io(const std::string& devname, unsigned int baud_rate,
    ...
    */
    ...
    /// Asynchronous read, the user must call poll() or run_one
    void read_string_until_async(const std::string& delim="\n");
    boost::asio::io_service& io_service() { return io_; }
    bool check_async( std::string& line );
...
private:
     ...
     */
    class read_setup_parameters
    {
    public:
        read_setup_parameters(): fixed_size(false), delim(""), data(0), size(0) {}

        explicit read_setup_parameters(const std::string& delim):
                fixed_size(false), delim(delim), data(0), size(0) { }

        read_setup_parameters(char *data, size_t size): fixed_size(true),
                delim(""), data(data), size(size) { }

        //Using default copy constructor, operator=

        bool fixed_size; ///< True if need to read a fixed number of parameters
        std::string delim; ///< String end delimiter (valid if fixed_size=false)
        char *data; ///< Pointer to data array (valid if fixed_size=true)
        size_t size; ///< Array size (valid if fixed_size=true)
    };
     ...
     */
    void perform_read_setup(const read_setup_parameters& param);
     ...
     */
    void read_completed(const boost::system::error_code& error,
    ...
    boost::asio::streambuf read_data_; ///< Holds eventual read but not consumed
    enum read_result result_;  ///< Used by read with timeout
    size_t bytes_transferred_; ///< Used by async read callback
    read_setup_parameters setup_parameters_; ///< Global because used in the OSX fix
};
} }  // namespace snark { namespace ocean {

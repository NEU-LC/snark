...
namespace {
    ...
    void usage( bool verbose = false )
    {
        ...
        std::cerr << "        perform conversion between rgb, ycbcr, and other colorspaces on input streams" << std::endl;
        std::cerr << std::endl;
        std::cerr << "    colorspace names:" << std::endl;
        std::cerr << "        rgb     - red-green-blue" << std::endl;
        std::cerr << "        ycbcr   - luma and chroma" << std::endl;
        std::cerr << std::endl;
        std::cerr << "    precision ranges:" << std::endl;
        std::cerr << "        color values can be stored in variables of different precision and may use different intervals of values" << std::endl;
        std::cerr << "        although some sane defaults can be assumed, the user can specify the range explicitly; range is given as" << std::endl;
        std::cerr << "            <precision>[:<from>-<to>]" << std::endl;
        std::cerr << "        here precision is one of the standard precision formats (b,ub,w,uw,i,ui,f,d) and" << std::endl;
        std::cerr << "        from and to are the the minimal and the maximal value possible for this channel, e.g.:" << std::endl;
        std::cerr << "            ub:16-235 describes single-byte value with a footroom of 16 and headroom of 255 - 235 = 20" << std::endl;
        std::cerr << "            f:0-255 describes floating point value with non-default range corresponding to ub" << std::endl;
        std::cerr << "        default ranges are:" << std::endl;
        std::cerr << "            min-max for integer values, as defined by numeric limits for the respective type" << std::endl;
        std::cerr << "            0-1 for floating-point values" << std::endl;
        std::cerr << "        ranges can be appended to --from,--to options, or given by dedicated options" << std::endl;
        std::cerr << std::endl;
        std::cerr << "options" << std::endl;
        std::cerr << std::endl;
        std::cerr << "convert options" << std::endl;
        std::cerr << "    --from=[<colorspace>[:range]]; input colorspace and range, optional, alternatively can be inferred from fields" << std::endl;
        std::cerr << "    --to=<colorspace>[:range]; destination colorspace, mandatory" << std::endl;
        std::cerr << "    --input-range=[<settings>]; explicitly specify the expected range of input values; use if the from colorspace is inferred from fields" << std::endl;
        std::cerr << "    --output-range=[<settings>]; explicitly specify the expected range of output values" << std::endl;
        ...
        std::cerr << std::endl;
        std::cerr << "    convert" << std::endl;
        std::cerr << "        rgb to ycbcr; explicit range defines input and output as 8-bit digital values" << std::endl;
        std::cerr << "            echo 1,2,3 \\" << std::endl;
        std::cerr << "                | image-color-calc --from rgb:ub --to ycbcr:ub" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        same direction but input is analog, a value in non-default range from -1 to 1" << std::endl;
        std::cerr << "            echo 1,0.2,-0.3 \\" << std::endl;
        std::cerr << "                | image-color-calc --from rgb:f:-1-1 --to ycbcr:ub" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        handle binary, same conversion as above" << std::endl;
        std::cerr << "            echo 1,0.2,0.3 | csv-to-bin 3f \\" << std::endl;
        std::cerr << "                | image-color-calc --from=rgb:f --to=ycbcr:ub \\" << std::endl;
        std::cerr << "                | csv-from-bin 3f,3ub" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        using fields to select values to convert, no --from needed; have to specify input range explicitly" << std::endl;
        std::cerr << "            echo 'value',128,128,128,20170101T000000 \\" << std::endl;
        std::cerr << "                | image-color-calc --fields=name,r,g,b,t --input-range=ub --to=ycbcr:ub" << std::endl;
        std::cerr << std::endl;
        ...
        std::cerr << "                | image-color-calc --fields=name,y,cb,cr,t --to rgb:ub --input-range=f:0-255" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        same example on binary data; format given in --input-range must match the binary format" << std::endl;
        std::cerr << "            echo 'value',30.5,40.2,50.3,20170101T000000 \\" << std::endl;
        std::cerr << "                | csv-to-bin s[10],3f,t | image-color-calc --fields=name,y,cb,cr,t --binary=s[10],3f,t --to rgb:ub --input-range=f:0-255\\" << std::endl;
        ...
        std::cerr << "                | image-color-calc --fields=name,channel0,channel1,channel2,t --from rgb:ub --to=ycbcr:f" << std::endl;
        std::cerr << std::endl;
        exit( 0 );
    }

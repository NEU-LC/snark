...
namespace {
    ...
    void usage( bool verbose = false )
    {
        ...
        std::cerr << "        color values can be stored in variables of different precision; although some sane defaults can be assumed," << std::endl;
        std::cerr << "        it is not possible in general; the user may have or want to specify the input format explicitly as" << std::endl;
        std::cerr << "            <precision>[:range]" << std::endl;
        std::cerr << "        here both precision and optional range are standard format abbreviations (b, ub, w, uw, etc.)" << std::endl;
        std::cerr << "        precision decribes the value storage format, while range gives the type of the value being stored" << std::endl;
        std::cerr << "        e.g., f:ub describes a floating point value that contains ub data, say, rgb of [128.2,129.3,255.1]" << std::endl;
        std::cerr << "        the input would be read as float but used in calculations as a value in ub range, keeping precision" << std::endl;
        std::cerr << "        by default range is the same as precision; precision must be higher than range, e.g., ub:f is not allowed" << std::endl;
        std::cerr << "        input format can be appended to the --from options, or specified separately by --input-format option" << std::endl;
        std::cerr << std::endl;
        std::cerr << "    output format:" << std::endl;
        std::cerr << "        usually the output is rounded to ub; alternative output format can be specified to increase precision" << std::endl;
        std::cerr << std::endl;
        std::cerr << "options" << std::endl;
        std::cerr << std::endl;
        std::cerr << "convert options" << std::endl;
        std::cerr << "    --from=[<colorspace>[:format]]; input colorspace and format, optional, alternatively can be inferred from fields" << std::endl;
        std::cerr << "    --to=<colorspace>[:format]; destination colorspace, mandatory" << std::endl;
        std::cerr << "    --input-fields; show input field names for the given --from colorspace and exit" << std::endl;
        std::cerr << "    --input-format=[<settings>]; explicitly specify the expected precision and range of input values" << std::endl;
        std::cerr << "      default: ub for rgb and ycbcr, f for ypbpr" << std::endl;
        std::cerr << "    --output-fields; show output field names for the given --to colorspace and exit" << std::endl;
        std::cerr << "    --output-format=[<format>]; default=ub; explicitly specify the desired format of output values" << std::endl;
        std::cerr << std::endl;
        ...
        std::cerr << std::endl;
        std::cerr << "    convert" << std::endl;
        std::cerr << "        rgb to ycbcr; explicit formats define input and output as 8-bit digital values" << std::endl;
        std::cerr << "            echo 1,2,3 \\" << std::endl;
        std::cerr << "                | image-color-calc --from rgb:ub --to ycbcr:ub" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        same direction but input is analog, a value from 0 to 1 and output is not rounded (but still in 0-255 interval)" << std::endl;
        std::cerr << "            echo 1,0.2,0.3 \\" << std::endl;
        std::cerr << "                | image-color-calc --from rgb:f --to ycbcr:f" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        handle binary, same conversion as above" << std::endl;
        std::cerr << "            echo 1,0.2,0.3 | csv-to-bin 3f \\" << std::endl;
        std::cerr << "                | image-color-calc --from=rgb:f --to=ycbcr:f \\" << std::endl;
        std::cerr << "                | csv-from-bin 3f,3f" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        using fields to select values to convert, no --from needed; have to specify input format explicitly" << std::endl;
        std::cerr << "            echo 'value',128,128,128,20170101T000000 \\" << std::endl;
        std::cerr << "                | image-color-calc --fields=name,r,g,b,t --input-format=ub --to=ycbcr:ub" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        field names select conversion from ycbcr to rgb; use non-default range of input values" << std::endl;
        std::cerr << "            echo 'value',30.5,40.2,50.3,20170101T000000 \\" << std::endl;
        std::cerr << "                | image-color-calc --fields=name,y,cb,cr,t --to rgb:ub --input-format=f:ub" << std::endl;
        std::cerr << std::endl;
        std::cerr << "        same example on binary data; format given in --input-range must match the binary format" << std::endl;
        std::cerr << "            echo 'value',30.5,40.2,50.3,20170101T000000 \\" << std::endl;
        std::cerr << "                | csv-to-bin s[10],3f,t | image-color-calc --fields=name,y,cb,cr,t --binary=s[10],3f,t --to rgb:ub --input-range=f:ub\\" << std::endl;
        ...
        std::cerr << "                | image-color-calc --fields=name,channel0,channel1,channel2,t --from rgb:ub --to=ycbcr --output-format=d" << std::endl;
        std::cerr << std::endl;
        exit( 0 );
    }
